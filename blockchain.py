...
def mine_block():
    ...
    proof = proof_of_work()
    # Miners should be reward, so let's create a reward transaction
    # reward_transaction = {
    #     'sender': 'MINING',
    #     'recipient': owner,
...
while waiting_for_input:
    ...
    if user_choice == '1':
        ...
        if len(blockchain) >= 1:
            blockchain[0] = {
                'previous_hash': '',
                'index': 0,
                'transactions': [{'sender': 'Chris', 'recipient': 'Serhii', 'amount': 100.0}]
        }
    elif user_choice == 'q':
    ...
    if not verify_chain():
        ...
        break
    print('Balance of {}: {:6.2f}'.format('Serhii', get_balance('Serhii')))
else:
    print('User left!')
